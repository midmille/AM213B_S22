...
def RK4(N, dt, t, dydt, y0): 
    """
    This function implements the explicit 4 stage RK method for the solution to the 
    provided IVP. This scheme uses the butcher Tableu provided in the Midterm Question 1 for 
    AM 213B. 
    ...
    for k in range(N-1): 
        ...
        k1 = dydt(y[k,:], t[k])
        k2 = dydt(y[k,:] + dt*0.5*k1, t[k] + 0.5*dt)
        k3 = dydt(y[k,:] + dt*0.5*k2, t[k] + 0.5*dt)
        k4 = dydt(y[k,:] + dt*k3, t[k] + dt)

        ## [Solving for the next step in y.]
        y[k+1, :] = y[k,:] + dt*((1/6)*k1 + (1/3)*k2 + (1/3)*k3 + (1/6)*k4)
